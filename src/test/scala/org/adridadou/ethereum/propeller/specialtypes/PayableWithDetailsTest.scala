...
/**
  ...
  */
class EthPayableCallTest extends FlatSpec with Matchers with Checkers with SolidityConversionHelper {
  ...
  private def checkEncode(contractObject: EthPayableContract, address: EthAddress, seed: BigInt) = {
    val account = new EthAccount(seed.bigInteger)
    val beforeBalance = SolidityConversionHelper.facade.getBalance(address)
    contractObject.addressPayableFunc(account.getAddress).`with`(EthValue.ether(10)).get().getResult.get() shouldEqual account.getAddress

    SolidityConversionHelper.facade.getBalance(address) shouldBe beforeBalance.plus(EthValue.ether(10))

...
class PayableTest extends FlatSpec with Matchers with Checkers with SolidityConversionHelper {
  ...
  private def checkEncode(contractObject: PayableContract, address: EthAddress, seed: BigInt) = {
    val account = new EthAccount(seed.bigInteger)
    val beforeBalance = SolidityConversionHelper.facade.getBalance(address)
    contractObject.addressPayableFunc(account.getAddress).`with`(EthValue.ether(10)).get() shouldEqual account.getAddress
...
trait PayableContract {
  def addressPayableFunc(account: EthAddress): Payable[EthAddress]
}

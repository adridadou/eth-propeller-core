...
class SolidityCompilerTest extends FlatSpec with Matchers with Checkers {
  ...
  it should "compile a smart contract from a single file" in {
    val result = solidityCompiler.compileSrc(
      SoliditySource.from(new File("src/test/resources/contract2.sol"))
    )
    val details = result
      .findContract("myContract2")
      .orElseThrow(() => new EthereumApiException("myContract2 not found"))
    val entries = details.getAbi
    assertEquals(6, entries.size)
  }

  it should "compile a smart contract from a single file for a specified evm version" in {
    val evmVersion: Optional[EvmVersion] =
      Optional.of(new EvmVersion("constantinople"))
    val result = solidityCompiler.compileSrc(
      SoliditySource.from(new File("src/test/resources/contract2.sol")),
      evmVersion
    )
  ...
  it should "compile a smart contract from multiple files with import" in {
    val result = solidityCompiler.compileSrc(
      SoliditySource.from(new File("src/test/resources/c1.sol"))
    )
    val details = result
      .findContract("c1")
      .orElseThrow(() => new EthereumApiException("c1 not found"))
    val entries = details.getAbi
    assertEquals(3, entries.size)
  }

  it should "parse the result from truffle too" in {
    val result = CompilationResult.parse(
      IOUtils.toString(
        new FileInputStream("src/test/resources/SmartToken.json"),
        StandardCharsets.UTF_8
      )
    )
    val contract = result
      .findContract("default")
      .orElseThrow(
        () => new EthereumApiException("default contract not found!")
      )
    contract.getAbi
  }
}

...
class EventsTest extends FlatSpec with Matchers with Checkers {
  ...
  "Events" should "be observable from the ethereum network" in {
    (for (compiledContract <- ethereum
            .compile(contractSource)
            .findContract("contractEvents")
            .asScala;
          solidityEvent <- ethereum
      ...
      .orElseThrow(() => new EthereumApiException("something went wrong!"))
  }

  it should "work with a generic list as well" in {
    (for (compiledContract: SolidityContractDetails <- ethereum
            .compile(contractSource)
            .findContract("contractEvents")
            .asScala;
          solidityEvent <- ethereum

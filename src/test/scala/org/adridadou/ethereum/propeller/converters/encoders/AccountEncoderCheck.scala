...
import org.adridadou.ethereum.propeller.solidity.converters.encoders.AccountEncoder
...
/**
  ...
  */
class AccountEncoderCheck extends Checkers with Matchers {
  ...
  private def checkEncode(seed: BigInt) = {
    ...
    val propellerEncoder = new AccountEncoder()
    val ethjResult = EthData.of(ethjEncoder.encode(account.getAddress.address))
    val propellerResult = propellerEncoder.encode(account)
